...
# Extracts features from audio files for use in a neural network for speech recognition
# Author: Ben Tomsett
...
def framed_signal(signal, sample_rate, frame_length=20, overlap=7.5):
    """
...
def windowed_signal(frames):
    ...
    return frames * np.hanning(len(frames[0]))
...
def create_mel_filters(num_filters, n_fft, sample_rate):
    ...
    mel_points = np.linspace(
        low_freq_mel, high_freq_mel, num_filters + 2
    )  # +2 for the edges
...
def calculate_deltas(mfccs):
    ...
    for t in range(mfccs.shape[0]):
        delta_mfccs[t] = (
            np.dot(np.arange(-n, n + 1), padded_mfccs[t : t + 2 * n + 1]) / denominator
        )
